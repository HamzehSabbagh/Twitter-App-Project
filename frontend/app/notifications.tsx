...
export default function NotificationsScreen() {
  ...
  function openNotification(notification: NotificationItem) {
    ...
    if (notification.type === "direct_message") {
      const directMessageUsername = notification.profile?.username || notification.actor?.username;

      if (directMessageUsername) {
        router.push({
          pathname: "/messages/[username]",
          params: { username: directMessageUsername },
        });
      }
      return;
    }
  ...
  async function handleFollowRequestAction(notification: NotificationItem, action: "accept" | "decline") {
    const username = notification.profile?.username || notification.actor?.username;
    ...
    setActingOnRequestId(notification.id);

    try {
      const response = await authFetch(
        `${API_BASE_URL}/profile/follow-requests/${username}${action === "accept" ? "/accept" : ""}`,
        {
      ...
      await parseJsonResponse(response);
      await authFetch(`${API_BASE_URL}/notifications/${notification.id}/read`, {
        method: "POST",
      });
      await loadNotifications();
    } catch (followRequestError) {
      setError(
        followRequestError instanceof Error
          ? followRequestError.message
          : action === "accept"
            ? t("could_not_accept_follow_request", "Could not accept the follow request.")
            : t("could_not_decline_follow_request", "Could not decline the follow request.")
      );
    } finally {
                ...
                {item.type === "follow_request" ? (
                  <View className="mt-4 flex-row gap-2">
                    <Pressable
                      ...
                      disabled={actingOnRequestId === item.id}
                      className="rounded-full px-4 py-2 disabled:opacity-60"
                      style={{ backgroundColor: colors.primary }}
                    >
                      <Text className="text-xs font-semibold" style={{ color: colors.primaryText }}>
                        {actingOnRequestId === item.id ? t("loading", "Loading...") : t("accept", "Accept")}
                      </Text>
                    </Pressable>

                    <Pressable
                      onPress={() => handleFollowRequestAction(item, "decline")}
                      disabled={actingOnRequestId === item.id}
