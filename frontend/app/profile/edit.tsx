...
import { getCoverImageSource, getProfileImageSource } from "@/lib/profile-images";
...
const EXAMPLE_PROFILE = {
  ...
  bio: "Sharing thoughts, photos, and everyday updates.",
};
...
export default function EditProfileScreen() {
  ...
  const [pictureFile, setPictureFile] = useState<SelectedImage | null>(null);
  const [coverFile, setCoverFile] = useState<SelectedImage | null>(null);
    ...
    async function loadProfile() {
      ...
      try {
        ...
        const nextProfile = {
          ...
          picture_url: data.profile.picture_url ?? "",
  ...
  async function pickImage(kind: "picture" | "cover") {
    const permissionResult = await ImagePicker.requestMediaLibraryPermissionsAsync();

    if (!permissionResult.granted) {
      Alert.alert(
        t("permission_required", "Permission required"),
        t("media_permission_required", "Please allow photo library access to choose an image.")
      );
      return;
    }
  ...
  async function handleSave() {
    ...
    try {
      ...
      formData.append("is_profile_public", form.is_profile_public ? "1" : "0");
          ...
          {loading ? (
            ...
            <View className="mt-6 gap-4">
              ...
              >
                <Image
                  source={
                    coverFile
                      ? { uri: coverFile.uri }
                      : getCoverImageSource(initialProfile.cover_url, initialProfile.picture_url)
                  }
                  className="h-52 w-full"
                  resizeMode={coverFile || initialProfile.cover_url || initialProfile.picture_url ? "cover" : "contain"}
                />
                <View className="absolute bottom-4 left-4">
                  <Image
                    source={
                      pictureFile
                        ? { uri: pictureFile.uri }
                        : getProfileImageSource(initialProfile.picture_url)
                    }
              ...
              <View className="flex-row flex-wrap gap-2">
                ...
                >
                  <Text className="text-xs font-semibold" style={{ color: colors.text }}>
                    {t("change_profile_picture", "Change profile picture")}
                  </Text>
                </Pressable>
                <Pressable
                ...
                >
                  <Text className="text-xs font-semibold" style={{ color: colors.text }}>
                    {t("change_cover_photo", "Change cover photo")}
                  </Text>
                </Pressable>
              </View>

              <Text className="text-xs" style={{ color: colors.textMuted }}>
                {t("crop_hint", "You can crop profile photos as a square and cover photos as a larger wide banner before saving.")}
              </Text>
              ...
              <View>
                ...
                <TextInput
                  ...
                  placeholder={EXAMPLE_PROFILE.location}
                  placeholderTextColor={colors.textMuted}
                  className="mt-2 rounded-xl border px-4 py-3 text-sm"
                  style={{ borderColor: colors.inputBorder, backgroundColor: colors.inputBg, color: colors.text }}
                  textAlign={isRTL ? "right" : "left"}
              ...
              <View>
                ...
                <TextInput
                  ...
                  placeholder={EXAMPLE_PROFILE.bio}
                  placeholderTextColor={colors.textMuted}
